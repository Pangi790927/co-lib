...
than just being "the same idea in different syntax."
...
defined once per active block (the per-backend `io_desc_t` definitions in `colib.h`, then the
`io_pool_t`/`timer_pool_t` ones, in the order Unix, Linux, Windows, Unknown). Exactly one
`COLIB_OS_*` macro is meant to be true at a time - the header doesn't guard against more than one
being set, so setting two at once
...
or calls a method on it. Concretely, the three real shapes (the `struct io_desc_t` in each of
`colib.h`'s `#if COLIB_OS_LINUX`/`COLIB_OS_WINDOWS`/`COLIB_OS_UNIX` blocks) share nothing:
...
`add_waiter` (on the `#if COLIB_OS_LINUX` epoll `io_pool_t`) just does an `epoll_ctl`
`EPOLL_CTL_ADD`/`_MOD` - "wake me when this fd is readable/writable" - no `read`/`write` syscall
happens yet. `handle_ready()` (same `io_pool_t`, covered in `03_execution_model.md`'s "Where I/O and
timers fit in") is a no-op if `ready_tasks` already has entries, and otherwise blocks in
`epoll_wait(..., -1)` until the kernel reports something ready, then pushes the matching coroutines
onto `ready_tasks`. Only once the coroutine is later actually resumed does the wrapper function
(e.g. `colib::read` in the `#if COLIB_OS_LINUX` wrapper block) perform the real
`::read()`/`::write()` syscall, synchronously, trusting epoll's word that it won't block.
`force_awake` (same `io_pool_t`) pushes the waiting coroutine onto `ready_tasks` with the given
error, for cancellation. The `#if COLIB_OS_LINUX` `timer_pool_t` is `timerfd`-based - a timer is just
...
wrapper (e.g. `colib::ReadFile`) builds an `io_desc_t`/`io_data_t` and stashes a lambda in
`io_data_t::io_request` that, when invoked, calls the *real* `::ReadFile()` (or whichever WinAPI/
Winsock-extension function) immediately, passing the embedded `OVERLAPPED`. `add_waiter` (on the
Windows IOCP `io_pool_t`) is what actually invokes that stashed `io_request` - so "registering the
wait" *is* "issue the real async syscall right now." The coroutine then suspends until IOCP's
completion port reports the operation finished (`handle_ready()`/`handle_ready_events()` on the same
`io_pool_t`, via `GetQueuedCompletionStatusEx`), at which point the byte count/result is already
available - no second syscall afterward the way there is on Linux. `force_awake` (same `io_pool_t`)
cancels the pending overlapped operation. The `#if COLIB_OS_WINDOWS` `timer_pool_t` uses
`SetWaitableTimer`, tied into the same
...
`io_awaiter_t::await_suspend` only ever calls `do_wait_io_modifs` → `pool_internal_t::wait_io` →
(backend-specific `add_waiter`) → suspend → resume → `do_unwait_io_modifs`. Whether "registration"
merely arms a readiness watch
...
`io_pool_t::add_waiter` (on the `#if COLIB_OS_UNIX` kqueue `io_pool_t`) is implemented - it
registers a `kevent` the same readiness-based way epoll does (`EVFILT_READ`/`EVFILT_WRITE` instead
of `EPOLLIN`/`EPOLLOUT`).
**`force_awake` and `clear` are not**:
...
nothing on a `COLIB_OS_UNIX`/kqueue build.** The `#if COLIB_OS_UNIX` `timer_pool_t` is a stub too -
every one of `get_timer`/`set_timer`/`free_timer` is an empty body. `dbg_to_str(const io_desc_t&)`
for the kqueue variant is also unimplemented, returning a fixed
...
**The Linux and Unix wrapper blocks (`colib.h`'s `#if COLIB_OS_LINUX` and `#if COLIB_OS_UNIX`
wrapper-function blocks) are two separate, independent `#if` blocks - not shared via
`#if COLIB_OS_LINUX || COLIB_OS_UNIX` the way, say, `read_sz`/`write_sz` are (they live in the shared
`#if COLIB_OS_LINUX || COLIB_OS_UNIX` block).** They read as near-duplicates of each other - same
function names, same non-blocking-`connect()` dance, same control flow - and a line-count diff
confirms it:
