...
| 05 | `05_platforms.md` - Platforms: Windows/Linux backends, the WIP Unix/kqueue engine | not started | 2026-08-15 | - | `colib.h`'s epoll/IOCP/kqueue `io_pool_t`/`timer_pool_t` backends, `COLIB_OS_*` selection | Requested by the user 2026-08-15. Should account for `tests/BUGS.md` #4 (kqueue's `force_awake`/`clear` are unimplemented stubs) - the chapter needs to state that plainly, not imply three symmetric backends. |
