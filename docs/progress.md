...
| 05 | `05_platforms.md` - Platforms: Windows/Linux backends, the WIP Unix/kqueue engine | done | 2026-08-15 | `fd519a6` | `colib.h`'s epoll/IOCP/kqueue `io_pool_t`/`timer_pool_t` backends, `io_desc_t`'s three shapes, the wrapper-function blocks, `COLIB_OS_*` selection | Requested by the user 2026-08-15. States kqueue's incomplete status plainly (cross-referencing `tests/BUGS.md` #4/`TODO.md` #1) rather than implying three symmetric backends. Verified line-by-line, including a fresh finding: the Linux and Unix wrapper-function blocks (colib.h ~4658-4890) are two separate `#if` blocks, not shared via `#if LINUX \|\| UNIX` - confirmed via diff that they're near-duplicates *not* out of laziness, every difference is exactly where the two backends' `io_desc_t` shapes differ (epoll's `.fd`/`.events` vs kqueue's `.ident`/`.filter`). |
