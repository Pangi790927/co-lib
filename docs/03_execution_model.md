...
does with the coroutines it's handed, and where semaphores fit into that picture.
...
This is `task<T>`'s own awaiter (`task<T>::await_suspend()`). Calling is a *nested*
...
3. `CO_MODIF_INHERIT_ON_CALL`-flagged modifications are inherited from the caller into the callee's
   own modif table (`inherit_modifs()`).
...
(`final_awaiter_cleanup()`/`cpp_yield_awaiter()`): the relevant modifs fire, and
...
caller the same way, but they differ in one important respect. `co_yield value`
(`cpp_yield_awaiter()`) marks the callee's state with `err = ERROR_YIELDED` before transferring
back, and `task<T>::await_resume()` only destroys the callee's coroutine frame when
`err != ERROR_YIELDED` - so a `co_yield`, unlike a `co_return`, leaves the callee's frame alive and
...
Both forms end up in `pool_internal_t::sched()`:
...
- `pool->sched(task, v)` (the plain `pool_t` method, e.g. called from `main()`) has no calling
  coroutine to speak of, so `pool_t::sched` passes `nullptr` as the parent table. `inherit_modifs`
  returns immediately on a `nullptr` parent - nothing is inherited, only the modifs you passed
  explicitly apply.
- `co_await colib::sched(task, v)` (`sched_awaiter_t::await_suspend()`) runs from
...
continuing - follow the `sched` with `co_await colib::yield()` (`yield_awaiter_t`): it pushes the
*current* coroutine onto the back of the ready queue and immediately symmetric-transfers to
whatever's next in line. Since the freshly sched'd task was pushed onto that same queue first, it's
ahead in FIFO order and gets to run before this coroutine's own turn comes back around.
...
`pool_t::run()` just forwards to `pool_internal_t::run()`, which is a small loop:
...
`colib::yield()` (`yield_awaiter_t`) is what a call chain can use to opt out of
...
(`std::deque<state_t*>`, `pool_internal_t::ready_tasks`). Three things feed into it:
...
`next_task_state()` is what actually pops from it: `ready_tasks.pop_front()` if
...
`push_ready_front()`, used by `force_stop()` to jump its own continuation to the
...
when there's a reason to: if `ready_tasks` already has entries, `handle_ready()` is a no-op
(`io_pool_t::handle_ready()`, the epoll backend) - no point checking the OS if there's already work
queued. It's *only* when the ready queue is empty and there's at least one pending I/O wait that
the pool actually blocks (`epoll_wait(..., -1)` - infinite timeout) until something becomes ready.
Timers ride the same path: `sleep*()` registers an OS timer (`timerfd` on Linux, `SetWaitableTimer`
on Windows) as just another I/O wait, so a pending sleep is exactly what keeps this blocking wait
alive instead of returning immediately.
...
**`wait()`** (`sem_awaiter_t`): if the counter is already positive, it decrements
and returns immediately without suspending at all (`await_ready()`). Otherwise, the waiting
coroutine's state is pushed onto the semaphore's *own* wait list (`push_waiter()`) -
...
**`signal(inc)`** (`sem_internal_t::signal()`): adjusts the counter, then, as long
...
internal `_awake_one()` takes from the back - so the longest-waiting coroutine is
